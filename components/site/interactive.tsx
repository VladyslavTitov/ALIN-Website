...
import {ArrowRight,ArrowLeft,CalendarDays,Phone,Menu,X,Expand,Printer} from 'lucide-react';
...
export function FAQ({lang}:{lang:Lang}){return <Accordion type="single" collapsible className="faq-list">{faq[lang].map(([q,a],i)=><AccordionItem value={`question-${i}`} key={q}><AccordionTrigger>{q}</AccordionTrigger><AccordionContent>{a}</AccordionContent></AccordionItem>)}</Accordion>}
const photos=['reception','treatment','consultation'];
export function Gallery({lang}:{lang:Lang}){const t=ui[lang];const [selected,setSelected]=useState<number|null>(null);return <><div className="gallery-grid">{photos.map((photo,i)=><button className="gallery-item" key={photo} onClick={()=>setSelected(i)} aria-label={`${t.galleryNames[i]} – ${lang==='de'?'Bild vergrößern':'enlarge image'}`}><img src={`/images/${photo}-800.webp`} srcSet={`/images/${photo}-800.webp 800w, /images/${photo}-1400.webp 1400w`} sizes="(max-width:680px) 90vw, 33vw" width={800} height={533} alt={t.galleryAlt[i]} loading="lazy"/><span className="gallery-caption">{t.galleryNames[i]}<Expand aria-hidden="true"/></span></button>)}</div><Dialog open={selected!==null} onOpenChange={open=>{if(!open)setSelected(null)}}><DialogContent className="lightbox-dialog" showCloseButton={false} onKeyDown={e=>{if(e.key==='ArrowRight')setSelected(v=>v===null?0:(v+1)%photos.length);if(e.key==='ArrowLeft')setSelected(v=>v===null?0:(v+photos.length-1)%photos.length)}}><DialogClose className="dialog-close" aria-label={t.close}><X/></DialogClose><img src={`/images/${photos[selected??0]}-1400.webp`} width={1400} height={933} alt={t.galleryAlt[selected??0]}/><div className="lightbox-bottom"><div><DialogTitle>{t.galleryNames[selected??0]}</DialogTitle><DialogDescription className="small-note">{lang==='de'?`Bild ${(selected??0)+1} von ${photos.length}`:`Image ${(selected??0)+1} of ${photos.length}`}</DialogDescription></div><div className="lightbox-controls"><button className="icon-button" aria-label={lang==='de'?'Vorheriges Bild':'Previous image'} onClick={()=>setSelected(v=>((v??0)+photos.length-1)%photos.length)}><ArrowLeft/></button><button className="icon-button" aria-label={lang==='de'?'Nächstes Bild':'Next image'} onClick={()=>setSelected(v=>((v??0)+1)%photos.length)}><ArrowRight/></button></div></div></DialogContent></Dialog></>}
export function PrintButton({lang}:{lang:Lang}){return <button className="btn secondary" onClick={()=>window.print()}><Printer/>{ui[lang].print}</button>}
